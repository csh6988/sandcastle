# Structured run-failure evidence on runtime events

ADR 0028 makes **runtime events** the structured event model. Recovering a
failed run — especially a multi-repository **workflow board** task — needs more
than the coarse `run.error.message` text:
...
We extend the `run.error` **runtime event** with an optional, structured,
...
- **Additive on the existing seam, not a new mechanism.** We add an optional
  nested `recovery?` object to the `run.error` variant rather than a
...
- **Effect-free public types.** `RunFailureKind`
  ...
  passes.
- **Every field optional.** All recovery fields are optional and
  plain-serializable, so a `run.error` event without recovery evidence
  round-trips through the board store and renders without crashing. The store
  serializes runtime events generically.
- **Classification lives in one pure helper.** `src/RunFailureEvidence.ts`
  ...
  `infrastructure` decision consistent for consumers that only have a message.
...
- **A new `run-recovery` event alongside `run.error`.** Rejected: it splits the
  failure into two events consumers must correlate, and duplicates the
  `run.error` timing. A nested optional object on the existing variant is
  simpler.
