  ...
  query: async <Query extends CompanyQuery>(
    ...
    switch (query.type) {
      ...
      case "delivery-candidates.inspect":
        return DeliveryCandidateViewSchema.parse(
          queryValue,
        ) as unknown as CompanyQueryResult<Query>;
      case "delivery-candidates.list":
        return DeliveryCandidateViewSchema.array().parse(
          queryValue,
        ) as unknown as CompanyQueryResult<Query>;
      case "accepted-delivery-authority.inspect":
        return AcceptedDeliveryCandidateAuthoritySchema.parse(
          queryValue,
        ) as unknown as CompanyQueryResult<Query>;
  ...
  executeEnvelope: async <Command extends EnvelopeCommand>(
    ...
    if (parsed.success) {
      if (
        ...
        envelope.command.type.startsWith("delivery.release-operation.")
      ) {
        return {
          ...parsed.data,
          value: ReleaseOperationViewSchema.parse(parsed.data.value),
