...
import {
  ...
  ImprovementProposalDecideCommandInputSchema,
  ImprovementProposalRequestDecisionCommandInputSchema,
  ImprovementProposalReviseCommandInputSchema,
  ImprovementProposalTransitionCommandInputSchema,
  type ImprovementApplicationOperationView,
  type ImprovementProposalView,
} from "./improvement/improvementProposalContracts.js";
import {
...
export * from "./improvement/improvementProposalContracts.js";
export * from "./statistics/statisticsContracts.js";
...
export const CodeReviewManifestSchema = z
  .object({
    schemaVersion: z.literal(1),
    ...
    workspaceImportId: z.string().trim().min(1),
    ...
    selfCheck: z
      .object({
        ...
        logRefs: z.array(z.string().trim().min(1)).min(1),
        evidenceRefs: z.array(z.string().trim().min(1)).min(1),
      })
      .strict(),
...
export const CodeReviewViewSchema = z.object({
  id: z.string(),
  topicId: z.string(),
  manifest: CodeReviewManifestSchema,
  manifestHash: Sha256Schema,
  workspace: z.object({
    id: z.string(),
    operationKey: z.string(),
    state: z.enum(["intent", "ready", "blocked", "unknown"]),
    reviewerAiMemberId: z.string(),
    reviewerPositionId: z.string(),
    reviewerSessionId: z.string(),
    reviewNodeRunId: z.string(),
    providerId: z.string().nullable(),
    workspaceRef: z.string().nullable(),
    capabilitySnapshotHash: Sha256Schema.nullable(),
    independenceEvidenceHash: Sha256Schema.nullable(),
  ...
  authority: z
    .object({
      id: z.string(),
      qualityGateResultId: z.string(),
      workPackageVersionId: z.string(),
      sourceCommit: z.string(),
      diffHash: Sha256Schema,
      reviewerSessionId: z.string(),
      independenceEvidenceHash: Sha256Schema,
      createdAt: z.string().datetime(),
    })
    .nullable(),
  defects: z.array(
    z.object({
      ...
      obligation: z.unknown(),
      reworkWorkPackageVersionId: z.string().nullable(),
      status: z.enum(["open", "rework-created", "closed"]),
      createdAt: z.string().datetime(),
    }),
  ),
  integrationEligible: z.boolean(),
});

export type CodeReviewManifest = z.infer<typeof CodeReviewManifestSchema>;
export type CodeReviewView = z.infer<typeof CodeReviewViewSchema>;

const IntegrationDependencySchema = z
  .object({
    predecessorWorkPackageVersionId: z.string().trim().min(1),
    kind: z.enum(["artifact", "commit", "contract", "readiness", "manual"]),
...
export const IntegrationGenerationManifestSchema = z
  .object({
    schemaVersion: z.literal(1),
    ...
    packages: z.array(IntegrationCoveragePackageSchema),
    dependencyOrder: z.array(z.string().trim().min(1)),
    contractVersions: z.array(
      z
        .object({
          id: z.string().trim().min(1),
...
export const IntegrationGenerationViewSchema = z
  .object({
    id: z.string().trim().min(1),
    manifest: IntegrationGenerationManifestSchema,
    manifestHash: Sha256Schema,
    state: z.enum([
      "pending",
      "running",
    ...
    repositoryResults: z.array(
      z
        .object({
          ...
          expectedTip: z.string().regex(/^[a-f0-9]{40}$/),
          integratedCommit: z
            .string()
            .regex(/^[a-f0-9]{40}$/)
            .nullable(),
...
export type IntegrationGenerationManifest = z.infer<
  typeof IntegrationGenerationManifestSchema
>;
export type IntegrationGenerationView = z.infer<
  ...
  .object({
    ...
    evidencePolicy: z
      .object({
        ...
        requiredKinds: z.array(z.string().trim().min(1)),
      })
      .strict(),
    cleanup: z
      .object({
...
const TestRunManifestInputSchema = z
  .object({
...
const TestAssertionCorrelationSchema = z
  .object({
    ...
    nodeAttemptId: z.string().trim().min(1),
    sessionId: z.string().trim().min(1),
    artifactVersionIds: z.array(z.string().trim().min(1)),
  })
  .strict();

const TestDefectResponsibilitySchema = z.discriminatedUnion("kind", [
  z
    .object({
  ...
  z
    .object({
      kind: z.literal("ui-runtime-contract"),
...
export const CriticalRiskEscalationDecisionSchema = z
  .object({
    id: z.string().trim().min(1),
    candidateInputId: z.string().trim().min(1),
    candidateInputHash: Sha256Schema,
    decision: z.enum(["authorize-gate-continuation", "reject"]),
    actor: ActorRefSchema.extend({
      type: z.literal("human"),
      authenticatedBy: z.literal("local-session"),
    }),
    risk: z.unknown(),
    riskHash: Sha256Schema,
...
>;
export type CriticalRiskEscalationDecisionView = z.infer<
  typeof CriticalRiskEscalationDecisionSchema
>;
export type CandidateQualityGateView = z.infer<
  typeof CandidateQualityGateViewSchema
>;

export const HumanReleaseDecisionViewSchema = z
  .object({
    id: z.string().trim().min(1),
...
export const AcceptedDeliveryCandidateAuthoritySchema = z
  .object({
    id: z.string().trim().min(1),
    candidateId: z.string().trim().min(1),
    candidateHash: Sha256Schema,
    ...
    candidateInputId: z.string().trim().min(1),
    candidateInputHash: Sha256Schema,
    gateAuthorityId: z.string().trim().min(1),
...
export type AcceptedDeliveryCandidateAuthorityView = z.infer<
  typeof AcceptedDeliveryCandidateAuthoritySchema
>;
...
export const StatisticsInspectQuerySchema = z
  .object({
    type: z.literal("statistics.inspect"),
    projectId: z.string().trim().min(1),
    query: StatisticsInspectInputSchema,
  })
  .strict();

export const StatisticsEvidenceInspectQuerySchema = z
  .object({
    type: z.literal("statistics-evidence.inspect"),
    evidenceSnapshotId: z.string().trim().min(1),
  })
  .strict();

export const ImprovementProposalInspectQuerySchema = z
  .object({
    type: z.literal("improvement-proposal.inspect"),
...
export const ImprovementApplicationInspectQuerySchema = z
  .object({
    type: z.literal("improvement-application.inspect"),
    operationId: z.string().trim().min(1),
  })
  .strict();

export const ImprovementApplicationListQuerySchema = z
  .object({
    type: z.literal("improvement-applications.list"),
    projectId: z.string().trim().min(1),
  })
  .strict();
...
export const CompanyQuerySchema = z.discriminatedUnion("type", [
  ...
  z.object({
    type: z.literal("code-reviews.inspect"),
    runId: z.string().trim().min(1),
  }),
  z.object({
    type: z.literal("integration-generations.inspect"),
    runId: z.string().trim().min(1),
  }),
  z.object({
    type: z.literal("test-runs.inspect"),
    testRunId: z.string().trim().min(1),
  }),
  z.object({
  ...
  z.object({
    type: z.literal("delivery-candidates.inspect"),
    candidateId: z.string().trim().min(1),
  }),
  z.object({
    type: z.literal("delivery-candidates.list"),
    runId: z.string().trim().min(1),
  }),
  z.object({
  ...
  StatisticsInspectQuerySchema,
  StatisticsEvidenceInspectQuerySchema,
  ImprovementProposalInspectQuerySchema,
  ImprovementProposalListQuerySchema,
  ImprovementApplicationInspectQuerySchema,
  ImprovementApplicationListQuerySchema,
  z.object({
...
export type ImprovementProposalQuery =
  | z.infer<typeof ImprovementProposalInspectQuerySchema>
  | z.infer<typeof ImprovementProposalListQuerySchema>;

export type ImprovementProposalQueryResult<
  Query extends ImprovementProposalQuery,
> = Query["type"] extends "improvement-proposal.inspect"
  ? ImprovementProposalView
  : readonly ImprovementProposalView[];

export type StatisticsQuery =
  | z.infer<typeof StatisticsInspectQuerySchema>
  | z.infer<typeof StatisticsEvidenceInspectQuerySchema>;

export type StatisticsQueryResult<Query extends StatisticsQuery> =
  Query["type"] extends "statistics.inspect"
    ? StatisticsView
    : StatisticsEvidenceSnapshotView;

export type ImprovementApplicationQuery =
  | z.infer<typeof ImprovementApplicationInspectQuerySchema>
  | z.infer<typeof ImprovementApplicationListQuerySchema>;

export type ImprovementApplicationQueryResult<
  Query extends ImprovementApplicationQuery,
> = Query["type"] extends "improvement-application.inspect"
                  ...
                  : Query["type"] extends "applications.list"
                    ...
                    : Query["type"] extends "review.topic.inspect"
                      ...
                      : Query["type"] extends "review.topics.list"
                        ...
                        : Query["type"] extends "code-reviews.inspect"
                          ? readonly CodeReviewView[]
                          : Query["type"] extends "integration-generations.inspect"
                              ...
                              : Query["type"] extends "test-pass-authority.inspect"
                                ...
                                : Query["type"] extends "delivery-candidate-input.inspect"
                                  ...
                                  : Query["type"] extends "quality-gates.inspect"
                                    ...
                                    : Query["type"] extends "delivery-candidates.inspect"
                                      ? DeliveryCandidateView
                                      : Query["type"] extends "delivery-candidates.list"
                                        ...
                                        : Query["type"] extends "accepted-delivery-authority.inspect"
                                          ...
                                          : Query["type"] extends "release-operations.inspect"
                                            ...
                                            : Query["type"] extends "release-operations.list"
                                              ...
                                              : Query["type"] extends "statistics.inspect"
                                                ? StatisticsView
                                                : Query["type"] extends "statistics-evidence.inspect"
                                                  ? StatisticsEvidenceSnapshotView
                                                  : Query["type"] extends "improvement-proposal.inspect"
                                                    ? ImprovementProposalView
                                                    : Query["type"] extends "improvement-proposals.list"
                                                      ? readonly ImprovementProposalView[]
                                                      : Query["type"] extends "improvement-application.inspect"
                                                        ? ImprovementApplicationOperationView
                                                        : Query["type"] extends "improvement-applications.list"
                                                          ? readonly ImprovementApplicationOperationView[]
...
export const CodeReviewStartEnvelopeCommandSchema = z
  .object({
    type: z.literal("code-review.start"),
    codeReviewId: z.string().trim().min(1),
    topicId: z.string().trim().min(1),
    workPackageId: z.string().trim().min(1),
    diffArtifactVersionId: z.string().trim().min(1),
...
export const CodeReviewConvergeEnvelopeCommandSchema = z
  .object({
    type: z.literal("code-review.converge"),
    codeReviewId: z.string().trim().min(1),
    reworkVersionId: z.string().trim().min(1).optional(),
    reworkBaseCommit: z
      .string()
      .regex(/^[a-f0-9]{40}$/)
      .optional(),
...
export type CodeReviewEnvelopeCommand =
  | z.infer<typeof CodeReviewStartEnvelopeCommandSchema>
  | z.infer<typeof CodeReviewConvergeEnvelopeCommandSchema>;

export const IntegrationGenerationStartEnvelopeCommandSchema = z
  .object({
    type: z.literal("integration.generation.start"),
    generationId: z.string().trim().min(1),
    runId: z.string().trim().min(1),
    nodeRunId: z.string().trim().min(1),
  })
...
export const IntegrationValidationRecordEnvelopeCommandSchema = z
  .object({
    ...
    kind: z.enum(["build-test", "contract"]),
    evidenceRefs: z.array(z.string().trim().min(1)).min(1),
    responsibleWorkPackageVersionIds: z.array(z.string().trim().min(1)),
    contractFailure: z
      .object({
        producerApplicationId: z.string().trim().min(1),
        consumerApplicationId: z.string().trim().min(1),
        ...
        fixtureRef: z.string().trim().min(1),
        runtimeEvidenceRef: z.string().trim().min(1),
      })
      .strict()
      .optional(),
  })
...
export const IntegrationAggregateReviewRecordEnvelopeCommandSchema = z
  .object({
    type: z.literal("integration.aggregate-review.record"),
    generationId: z.string().trim().min(1),
    topicId: z.string().trim().min(1),
    qualityGateResultId: z.string().trim().min(1),
  })
...
export type IntegrationEnvelopeCommand =
  | z.infer<typeof IntegrationGenerationStartEnvelopeCommandSchema>
  | z.infer<typeof IntegrationValidationRecordEnvelopeCommandSchema>
  | z.infer<typeof IntegrationAggregateReviewRecordEnvelopeCommandSchema>;

export const TestCaseRevisionRegisterEnvelopeCommandSchema = z
  .object({
    type: z.literal("test.case-revision.register"),
    testCaseId: z.string().trim().min(1),
    revisionId: z.string().trim().min(1),
    projectId: z.string().trim().min(1),
    supersedesRevisionId: z.string().trim().min(1).optional(),
    manifest: TestCaseRevisionManifestInputSchema,
  })
  .strict();

export const TestRunCreateEnvelopeCommandSchema = z
  .object({
    type: z.literal("test.run.create"),
...
export const TestDefectCloseEnvelopeCommandSchema = z
  .object({
...
export const CriticalRiskEscalationDecideEnvelopeCommandSchema = z
  .object({
    type: z.literal("quality-gate.critical-escalation.decide"),
...
export const DeliveryCandidateAssembleEnvelopeCommandSchema = z
  .object({
    type: z.literal("delivery.candidate.assemble"),
    candidateId: z.string().trim().min(1),
    requestId: z.string().trim().min(1),
    candidateInputId: z.string().trim().min(1),
    expectedCandidateInputHash: Sha256Schema,
    expectedGateAuthorityHash: Sha256Schema,
    nodeRunId: z.string().trim().min(1),
    nodeAttemptId: z.string().trim().min(1),
...
export const HumanReleaseDecideEnvelopeCommandSchema = z
  .object({
    type: z.literal("delivery.release.decide"),
    decisionId: z.string().trim().min(1),
    candidateId: z.string().trim().min(1),
    ...
    evidenceRefs: z.array(z.string().trim().min(1).max(512)).min(1).max(64),
    rework: HumanReleaseDecisionViewSchema.shape.rework.unwrap().optional(),
  })
  .strict();
...
export const StatisticsEvidenceFreezeEnvelopeCommandSchema = z
  .object({
    type: z.literal("statistics.evidence.freeze"),
    evidenceSnapshotId: z.string().trim().min(1),
    query: StatisticsInspectInputSchema,
  })
  .strict();

export const ImprovementProposalCreateEnvelopeCommandSchema = z
  .object({
    type: z.literal("improvement.proposal.create"),
    proposal: ImprovementProposalCreateCommandInputSchema,
  })
  .strict();

export const ImprovementProposalReviseEnvelopeCommandSchema = z
  .object({
    type: z.literal("improvement.proposal.revise"),
    proposal: ImprovementProposalReviseCommandInputSchema,
  })
  .strict();

export const ImprovementProposalProposeEnvelopeCommandSchema =
  ImprovementProposalTransitionCommandInputSchema.extend({
    type: z.literal("improvement.proposal.propose"),
  }).strict();

export const ImprovementProposalRequestDecisionEnvelopeCommandSchema =
  ImprovementProposalRequestDecisionCommandInputSchema.extend({
    type: z.literal("improvement.proposal.request-decision"),
  }).strict();

export const ImprovementProposalDecideEnvelopeCommandSchema =
  ImprovementProposalDecideCommandInputSchema.extend({
    type: z.literal("improvement.proposal.decide"),
  }).strict();

export const ImprovementApplicationApplyEnvelopeCommandSchema = z
  .object({
...
export const ImprovementApplicationValidateEnvelopeCommandSchema = z
  .object({
    type: z.literal("improvement.application.validate"),
    validation: ImprovementApplicationValidateCommandInputSchema,
  })
  .strict();

export const ImprovementApplicationRollbackEnvelopeCommandSchema = z
  .object({
    type: z.literal("improvement.application.rollback"),
    rollback: ImprovementApplicationRollbackCommandInputSchema,
  })
  .strict();

export type ImprovementEnvelopeCommand =
  | z.infer<typeof StatisticsEvidenceFreezeEnvelopeCommandSchema>
  | z.infer<typeof ImprovementProposalCreateEnvelopeCommandSchema>
  | z.infer<typeof ImprovementProposalReviseEnvelopeCommandSchema>
  | z.infer<typeof ImprovementProposalProposeEnvelopeCommandSchema>
  | z.infer<typeof ImprovementProposalRequestDecisionEnvelopeCommandSchema>
  | z.infer<typeof ImprovementProposalDecideEnvelopeCommandSchema>
  | z.infer<typeof ImprovementApplicationApplyEnvelopeCommandSchema>
  | z.infer<typeof ImprovementApplicationValidateEnvelopeCommandSchema>
  | z.infer<typeof ImprovementApplicationRollbackEnvelopeCommandSchema>;
...
export type DeliveryQualityEnvelopeCommand =
  ...
  | z.infer<typeof CriticalRiskEscalationDecideEnvelopeCommandSchema>;
...
export const EnvelopeCommandSchema = z.discriminatedUnion("type", [
  ...
  CodeReviewStartEnvelopeCommandSchema,
  CodeReviewConvergeEnvelopeCommandSchema,
  IntegrationGenerationStartEnvelopeCommandSchema,
  IntegrationValidationRecordEnvelopeCommandSchema,
  ...
  StatisticsEvidenceFreezeEnvelopeCommandSchema,
  ImprovementProposalCreateEnvelopeCommandSchema,
  ImprovementProposalReviseEnvelopeCommandSchema,
  ImprovementProposalProposeEnvelopeCommandSchema,
  ImprovementProposalRequestDecisionEnvelopeCommandSchema,
  ImprovementProposalDecideEnvelopeCommandSchema,
  ImprovementApplicationApplyEnvelopeCommandSchema,
  ImprovementApplicationValidateEnvelopeCommandSchema,
  ImprovementApplicationRollbackEnvelopeCommandSchema,
  ArtifactRegisterEnvelopeCommandSchema,
  ArtifactFinalizeEnvelopeCommandSchema,
  ArtifactSupersedeEnvelopeCommandSchema,
            ...
            : Command["type"] extends "delivery.candidate-input.authorize"
              ...
              : Command["type"] extends "quality-gate.critical-escalation.decide"
                ...
                : Command["type"] extends ReleaseOperationEnvelopeCommand["type"]
                  ...
                  : Command["type"] extends "statistics.evidence.freeze"
                    ? StatisticsEvidenceSnapshotView
                    : Command["type"] extends
...
export const EventEnvelopeSchema = z.object({
  ...
  statisticsEvidenceSnapshotId: z.string().optional(),
