...
const markWorkPackageAttemptReconciling = (
  fixture: ReturnType<typeof setup>,
  assignment: WorkPackageGraphView["packages"][number]["versions"][number]["assignments"][number],
) => {
    ...
    raw
      .prepare(
        `UPDATE node_attempts
            SET status = 'reconciling', execution_operation_key = ?,
                lease_id = NULL, lease_owner = NULL, lease_expires_at = NULL
          WHERE id = ?`,
      )
      .run(
        `work-package-reconcile:${assignment.nodeAttemptId}`,
      ...
      .prepare(
        `UPDATE node_runs
            SET status = 'blocked',
                ...
                failure_message = 'Provider terminal state requires reconciliation.'
          WHERE id = ?`,
      )
      .run(fixture.nodeRunIds.a);
    raw
      .prepare(
...
describe("Work Package Runtime", () => {
    ...
    try {
      ...
      assert.equal(
        fixture.database.pipelineRuntime
          ...
          .some((event) => event.type === "run.blocked"),
        true,
      );
  ...
  it("projects a reconciled failed Attempt onto its exact Work Package", async () => {
    const adapter: ExecutionAdapter = {
      execute: async () => ({
        kind: "failed",
        code: "UNEXPECTED_EXECUTION",
        message: "The reconciliation test must not execute a new Attempt.",
      }),
      reconcile: async (_input, sink) => {
        const receipt = await sink.record({
          adapterSchemaVersion: 1,
          factId: "work-package-reconciled-failure",
          ordinal: 1,
          kind: "failed",
          schemaVersion: 1,
          payload: {
            code: "PROVIDER_RECONCILED_FAILURE",
            message: "Provider evidence proves the isolated execution failed.",
          },
          evidenceRefs: ["provider-receipt:work-package-failed"],
        });
        return {
          status: "failed",
    ...
    try {
      assert.equal(
        execute(fixture.database, "generate-reconciled-failure", 0, {
          type: "work-package.generate",
          runId: fixture.runId,
          technicalBaselineId: fixture.technicalBaselineId,
          packages: [packageContracts(fixture)[0]],
      ...
      fixture.database.workspaces.executeProvision(assignment.allocationId);
      assert.equal(
        execute(fixture.database, "start-reconciled-failure", 1, {
          type: "work-package.start",
          workPackageId: "package-a",
        }).status,
        "succeeded",
      );
      markWorkPackageAttemptReconciling(fixture, assignment);

      assert.equal(
        await fixture.database.pipelineRuntime.reconcilePendingExecutions(),
        1,
      );
      const graph = fixture.database.workPackages.inspect(fixture.runId);
      assert.equal(graph.packages[0]!.state, "failed");
      assert.equal(
        graph.packages[0]!.versions[0]!.assignments[0]!.state,
        "failed",
      );
      const run = fixture.database.pipelineRuntime.inspectRun(fixture.runId);
      assert.equal(run.run.status, "blocked");
      assert.equal(
        run.nodes.find((node) => node.id === fixture.nodeRunIds.a)!.attempts[0]!
          .status,
  ...
  it("projects a reconciled not-started Attempt as interrupted while preserving the blocked Run", async () => {
    const adapter: ExecutionAdapter = {
      execute: async () => ({
        kind: "failed",
        code: "UNEXPECTED_EXECUTION",
      ...
      reconcile: async (_input, sink) => {
        const receipt = await sink.record({
          adapterSchemaVersion: 1,
          factId: "work-package-reconciled-not-started",
          ordinal: 1,
          kind: "not-started",
          schemaVersion: 1,
          payload: {
            ...
            message:
              "Provider evidence proves the isolated execution never started.",
            providerReceipt: "provider-receipt:work-package-not-started",
          },
          evidenceRefs: ["provider-receipt:work-package-not-started"],
        });
        return {
          status: "not-started",
          terminalExecutionFactId: receipt.executionFactId,
    ...
    try {
      assert.equal(
        execute(fixture.database, "generate-reconciled-not-started", 0, {
          type: "work-package.generate",
          runId: fixture.runId,
          technicalBaselineId: fixture.technicalBaselineId,
          packages: [packageContracts(fixture)[0]],
      ...
      fixture.database.workspaces.executeProvision(assignment.allocationId);
      assert.equal(
        execute(fixture.database, "start-reconciled-not-started", 1, {
          type: "work-package.start",
          workPackageId: "package-a",
        }).status,
        "succeeded",
  ...
  it("imports the exact commit before projecting a reconciled completed Attempt to self-check", async () => {
    ...
    const adapter: ExecutionAdapter = {
      ...
      reconcile: async (_input, sink) => {
        const receipt = await sink.record({
          adapterSchemaVersion: 1,
          factId: "work-package-reconciled-completion",
          ordinal: 1,
          kind: "completed",
          schemaVersion: 1,
          payload: {
            structuredResult: { commits: [{ sha: reconciledCommit }] },
          },
          evidenceRefs: ["provider-receipt:work-package-completed"],
        });
        return {
          status: "succeeded",
          terminalExecutionFactId: receipt.executionFactId,
    ...
    try {
      assert.equal(
        execute(fixture.database, "generate-reconciled-completion", 0, {
          type: "work-package.generate",
          runId: fixture.runId,
          technicalBaselineId: fixture.technicalBaselineId,
          packages: [packageContracts(fixture)[0]],
      ...
      const assignment =
        assigned.value.packages[0]!.versions[0]!.assignments[0]!;
      const provisioned = fixture.database.workspaces.executeProvision(
        assignment.allocationId,
      );
      assert.equal(
        execute(fixture.database, "start-reconciled-completion", 1, {
          type: "work-package.start",
          workPackageId: "package-a",
        }).status,
        "succeeded",
      );
      const executionTreePath = (
        provisioned.provisionReceipt as {
          readonly executionTreePath: string;
        }
      ).executionTreePath;
      writeFileSync(
        join(executionTreePath, "reconciled.txt"),
        "reconciled completion\n",
      );
      git(executionTreePath, "add", "reconciled.txt");
      git(
        executionTreePath,
        "-c",
        "user.name=Work Package Test",
        ...
        "commit",
        "-m",
        "reconcile isolated result",
      );
      reconciledCommit = git(executionTreePath, "rev-parse", "HEAD");
      markWorkPackageAttemptReconciling(fixture, assignment);
      ...
      const graph = fixture.database.workPackages.inspect(fixture.runId);
      assert.equal(graph.packages[0]!.state, "self-check");
      assert.equal(
        graph.packages[0]!.versions[0]!.assignments[0]!.state,
        "awaiting-self-check",
      );
      const imported =
        graph.packages[0]!.versions[0]!.assignments[0]!.allocation.imports[0]!;
      assert.equal(imported.state, "succeeded");
      assert.equal(imported.resultCommit, reconciledCommit);
      assert.equal(
        git(
          fixture.repositoryA.root,
          "rev-parse",
          assignment.allocation.sourceBranch,
        ),
        reconciledCommit,
      );
    } finally {
