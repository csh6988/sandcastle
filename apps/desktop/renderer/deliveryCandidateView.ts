...
type DeliveryCandidateBridge = Pick<
  SandcastleBridge,
  "query" | "execute" | "openEventStream" | "closeEventStream"
>;

export interface DeliveryCandidateConnection {
  readonly resync: () => Promise<void>;
  readonly close: () => Promise<void>;
}

export const connectDeliveryCandidate = async (input: {
  readonly bridge: DeliveryCandidateBridge;
  readonly candidateId: string;
  readonly onView: (view: DeliveryCandidateView) => void;
  readonly onDiagnostic: (diagnostic: string | null) => void;
}): Promise<DeliveryCandidateConnection> => {
  let closed = false;
  let generation = 0;
  ...
  const fail = async (error: unknown): Promise<void> => {
    input.onDiagnostic(
      error instanceof Error
        ? `Runtime unavailable; Delivery Candidate resync required: ${error.message}`
        : "Runtime unavailable; Delivery Candidate resync required.",
    );
    await closeActive();
  };
  const queryView = () =>
  ...
  const synchronize = async (): Promise<void> => {
    ...
    const opened = await input.bridge.openEventStream((frame) => {
      eventQueue = eventQueue.then(async () => {
        ...
        try {
          if (event.deliveryCandidateId === input.candidateId) {
