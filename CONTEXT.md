...
**Work Package**:
A formally versioned implementation contract produced from an approved **Technical Baseline**. Each version belongs to exactly one Application and Repository and declares its objective, acceptance criteria, dependencies, module scope, required permissions, assignment criteria, required `branch` strategy/isolation, expected Artifacts, and integration conditions. Each frozen `development@1` **Node run** has at most one active Work Package Version. The Version does not own concrete runtime resources: every **Node attempt** gets its own **Workspace allocation** and assigned AI member. Dependencies are producer-first, bind their exact Artifact/Attempt, import, Contract, readiness, or Human Approval authority, and prevent allocation until that evidence is satisfied. Developer self-check is structured evidence for later independent review, never approval, and successors of a succeeded package-bound Node remain queued until the current active Version's exact Assignment passes it; a graph-defined skipped branch remains satisfied. Cross-Repository goals use dependent Work Packages rather than one multi-write attempt.
...
**Integration branch**:
A dedicated, generation-scoped branch that assembles approved **Work Package** changes in declared dependency order. Each participating Repository has its own `integration/<run-id>/g<generation>` branch for an **Integration Generation**, created from that Repository's exact frozen common base. Only the Runtime-owned Integration Adapter may advance it, by applying an approved full base-to-source tree delta with an idempotent **Integration operation** and compare-and-swap; Developer, Reviewer, renderer, source importer, and Release operations cannot write it. The Integration branch is an intermediate delivery surface, not the user's final target branch.
...
An immutable, Project-level integration manifest for one bounded assembly attempt across all participating Repositories. It consumes one exact completed **Code Review coverage** authority and freezes its Run/Snapshot/Node Attempt identity, every Work Package Version/Authority/Gate tuple, one common base and generation branch per Repository, independently approved source commits and Diff hashes, producer-first dependency order, cross-application Contract versions, Integration conditions, and a canonical manifest hash. Any partial failure fails or blocks the whole generation; successful partial results remain evidence only, and recovery requires fresh reviewed inputs and a new generation. Only complete Repository validation, no open **Integration defect**, and an aggregate independent `PASS` over the exact integrated commits creates its downstream `passAuthorityHash`.
...
One idempotent application of an approved full base-to-source tree delta to a Repository's generation-scoped **Integration branch**, identified by an operation ID, Generation/manifest, Repository, source commit, expected Integration-branch tip, canonical request hash, and idempotency key. SQLite first records intent, the Runtime-owned Adapter performs the external Git effect outside that transaction, and a new unit of work records the exact receipt/final state. Identical replay returns the first receipt; changed input conflicts; restart reconciliation proves the result from exact Git identity or remains unknown/blocked without blind resend. It may produce an **Integration defect**, but it cannot modify the Developer's source branch, the user's **Target branch**, or bypass independent Code Review.
...
**Integration defect**:
A traceable defect raised when reviewed Work Package changes conflict, cannot be reconciled safely, fail Repository build/test validation, fail a cross-application Contract, or fail aggregate independent Review. It binds the Generation, optional Integration operation, affected Repository and commits, responsible or candidate Work Package Versions, producer/consumer Contract identity, fixture/Runtime evidence, and open/closed status. Repair returns through a fresh Work Package Attempt and independent Code Review, then a fresh Integration Generation; it is never fixed invisibly on the Integration branch.
...
The immutable execution contract captured atomically whenever confirmation or explicit fork formalizes a **department run**, covering the confirmed Product Baseline, department pipeline graph and Handler registry, positions, AI-member configuration, selected skills and skill flows, execution defaults, artifact contracts, and explicitly selected exact **Memory entries**. The later Start node only schedules formal Node runs. Later accepted Specs, readiness evidence, a **Technical Baseline**, or Memory selections enter downstream execution only through an appended **Snapshot revision**; live configuration never changes an existing revision.
_Avoid_: "current config" (mutable), "backup" (the snapshot is an execution contract), "copy" (does not express version identity)

**Snapshot revision**:
An immutable, integrity-identified revision of a **run configuration snapshot** used by one **department run**. The first revision is `r1`; a `PASS` **Quality Gate Result** may explicitly promote accepted Spec, readiness, or Technical Baseline inputs, and a verified human may explicitly select exact reviewed **Memory entries**, into a later revision linked to its parent, while every earlier revision remains unchanged and inspectable. A conditional or failed result cannot promote a Snapshot.
...
A formal review of one exact Work Package Version and its Runtime-imported source commit. One eligible non-producer records the initial independent finding, and a second distinct eligible non-producer supplies the fresh recheck from another new Session owned by that exact Code Review manifest and bound to the Code Review Node. Both operations execute under the shared Node Attempt Lease/fence and Execution Fact contract: message, Tool, usage, and terminal facts bind the Reviewer Session and aggregate Code Review Attempt, while the durable Reviewer stage freezes the exact structured result and actual Sandbox operation receipt. The Reviewer Execution Profile and non-sensitive Secret References/provider scopes are frozen separately from the producer allocation; overlap or an unavailable operation-local credential scope blocks execution. Rework manifests additionally freeze the prior Defect, Gate, findings, obligation, and evidence-backed resolution matrix. Independent Code Review produces evidence for a code-kind **Quality Gate Result** and cannot silently downgrade to self-review, bind-mount, no-sandbox, shared writable execution, shared producer credentials, or an unknown provider operation.
...
The immutable per-package Integration authority created only from a fresh independent code-kind `PASS` bound to the current exact Work Package Version, source commit, canonical Diff bytes/hash, fresh recheck Session, and both exact succeeded Reviewer execution stage IDs, isolation hashes, and result hashes, with no package-level open obligation. Each stage result must exactly match the persisted finding or recheck/Gate, successful stage evidence cannot be rewritten, and eligibility revalidates those bindings when queried. A rework obligation closes only when the new manifest froze its prior resolution matrix and the fresh recheck cites every required Defect, Gate, finding, obligation evidence, finding evidence, resolution identity, and resolution evidence reference.

**Code Review coverage**:
The aggregate internal authority of the shared `code-review@1` Node. Its succeeded Node and exact aggregate Node Attempt freeze the sorted current active Work Package Version IDs, their eligible Code Review authority IDs, their code-kind Quality Gate Result IDs, and a canonical `coverageHash`. The Node remains running until this set covers every active Version; Integration copies and revalidates this exact tuple set, rejecting stale or superseded authority instead of rescanning mutable package/review state or selecting a newer PASS.
...
**Test run**:
One recorded execution of one or more **Test cases** against a declared **Build Artifact**, Company Directory, **Execution Profile**, and Runtime fixture. Its immutable manifest binds the Build Artifact to the exact PASS **Integration Generation** and Repository commits, and preserves the Runtime-owned risk policy derived from the Snapshot's promoted **Technical Baseline**, deterministic factors and result, environment, inputs, UI actions, Runtime payloads, screenshots, logs, timing, and final status. The highest structured Work Package `riskTier` frozen by that Integration Generation is the minimum Test-scope tier; a missing or invalid tier fails closed, and incomplete Test Case coverage cannot down-tier the Run. Each assertion stores the exact UI and Runtime observations and derives pass/fail by canonical comparison with the frozen expected values. Runtime closes over the complete failed, unknown, and missing required-assertion union before terminalization: explicit failures make the Test run fail only when every non-passing member is an explicit failure, while any unknown or missing member blocks it, and every non-passing member creates deterministic rework authority. Execution intent, terminal facts, reconciliation evidence, and receipts remain durable so a result can be replayed or compared without relying on memory or a manually refreshed page.
...
**Test evidence**:
Immutable, retention-classified evidence from a **Test run** that correlates an exact Test Case revision and assertion with UI actions, command identity, Runtime event sequence, exact `test-runs.inspect` query, authenticated consumer/principal, completed acknowledgement receipt and audit/effect chain, the exact acknowledged predecessor token selected by the execution adapter, authoritative Query View hash and sequence, immutable observed values, and inspectable Artifact or payload references. A screenshot, label, older same-View token, or Agent statement alone is not sufficient evidence of authoritative Runtime behavior.
_Avoid_: mutable test logs, uncorrelated screenshots, Agent self-attestation, Renderer state as workflow truth

**Test defect**:
A traceable failure raised by a **Test run** against a requirement, Work Package, producer/consumer Contract, Artifact, Runtime contract, or interaction expectation. It links the failing Test case, exact evidence, affected revision and Node run when applicable, and either one evidenced responsibility or explicit aggregate/unknown candidates. Related unresolved lineage remains authoritative across Integration Generations within the same Project: Work Package identity follows newer Versions, producer/consumer Contract identity follows newer Contract versions, UI/Runtime ownership stays bound to the same Test Case ancestry, and aggregate/unknown candidates follow their Work Packages. The Defect is append-only and may have only one immutable evidence-backed resolution from the exact fresh passing **Test rework** run; resolution never rewrites the failed history.
...
**Test rework**:
A fresh **Test run** created only through the formal rework Command from a failed, blocked, or cancelled prior run. Rework-of-rework recursively unions all unresolved ancestor, current, and responsibility-related cross-Generation **Test defect** and obligation scope in the same Project. Its hash-bound lineage freezes every originating Test Run and Integration Generation, original revision/assertion/result hash, explicit successor revision/assertion mapping (including deliberate renames within the same Test Case ancestry), responsibility route, and whether it reuses the exact unchanged PASS **Integration Generation** or consumes a new PASS Generation after changed code or Integration inputs. One exact fresh PASS resolves the complete transitive scope atomically; direct creation and downstream authority both fail closed until that closure exists.
