...
A formal review of one exact Work Package Version and its Runtime-imported source commit. One eligible non-producer records the initial independent finding, and a second distinct eligible non-producer supplies the fresh recheck from another new Session owned by that exact Code Review manifest and bound to the Code Review Node. Both operations execute through durable Reviewer execution stages whose operation, participant, Session, structured result, and Sandbox isolation receipt must match before authority can be created. The Reviewer Execution Profile and non-sensitive Secret References are frozen separately from the producer allocation; an unavailable operation-local credential scope blocks execution. Rework manifests additionally freeze the prior Defect, Gate, findings, obligation, and resolution matrix. Independent Code Review produces evidence for a code-kind **Quality Gate Result** and cannot silently downgrade to self-review, bind-mount, no-sandbox, shared writable execution, shared producer credentials, or an unknown provider operation.
...
The immutable per-package Integration authority created only from a fresh independent code-kind `PASS` bound to the current exact Work Package Version, source commit, canonical Diff bytes/hash, fresh recheck Session, and both exact succeeded Reviewer execution receipts, with no package-level open obligation. A rework obligation closes only when the new manifest froze its prior resolution matrix and the fresh recheck cites every required Defect, Gate, and finding evidence reference. Eligibility is revalidated when queried rather than cached from the moment of approval.
