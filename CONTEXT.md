...
**Test rework**:
A fresh **Test run** created only through the formal rework Command from a failed, blocked, or cancelled prior run. Rework-of-rework recursively unions all unresolved ancestor and current **Test defect** and obligation scope. Its hash-bound lineage freezes every originating Test Run and Integration Generation, original revision/assertion/result hash, explicit successor revision/assertion mapping (including deliberate renames within the same Test Case ancestry), responsibility route, and whether it reuses the exact unchanged PASS **Integration Generation** or consumes a new PASS Generation after changed code or Integration inputs. One exact fresh PASS resolves the complete transitive scope atomically; partial mappings or closure leave no downstream authority.
