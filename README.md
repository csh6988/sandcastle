...
The Desktop Company Runtime provides the persistent local company workspace. Its **Agents** page detects formally registered local coding Agents and runs a safe adapter-specific probe without creating a Run; adapters with verified non-interactive contracts use ephemeral or non-persistent prompts. Its independent **Skills** page discovers `SKILL.md` files from the default or configured directories, supports ordered fuzzy search, reveals source references on demand, warns about declared `required-capabilities`, and tracks discovered, enabled, unavailable, and archived sources without copying Skill bodies. Review Topics freeze an exact input manifest and eligible-reviewer quorum, preserve independent findings through bounded discussion and revision, and require fresh Session votes; only an immutable unanimous `PASS` Quality Gate Result satisfies a downstream production contract. Product Review additionally binds the exact Product Baseline and Project Spec Revision, persists readiness evidence and reviewer-owned scope classification, and promotes the accepted Spec/readiness set into one immutable child Snapshot Revision only after `PASS`. Technical Review then binds registered Applications, exact Application Spec and cross-Application contract hashes, and an immutable Technical Baseline Proposal; only the Delivery coordinator can use a fresh `PASS` to atomically materialize the separate accepted Technical Baseline and append the next child Snapshot Revision. After exact PASS Test Runs, Runtime can freeze their complete authority into one immutable Delivery Candidate Input and run independent, risk-depth Security and Operability gates over its exact hash. The Reviews view reconstructs Candidate/Gate state from authoritative Query Views, and downstream authority remains blocked until both results are PASS with no open Defect or obligation; Delivery candidate creation and human release remain separate later actions.
